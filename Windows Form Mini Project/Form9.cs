    ...
    public partial class Forgot_Password : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (SqlConnection connection = new SqlConnection("data source = TEJAS\\SQL2025; database = .netSQL; user id = sa; password = 3938"))
            {
                ...
                string query = "SELECT password FROM wfproject WHERE username = @username AND first_name = @first_name AND last_name = @last_name AND email_id = @email_id AND phone_no = @phone_no";
                {
                    ...
                    if (result != null && result != DBNull.Value)
                    {
                        ...
                        MessageBox.Show("Your Password is: " + password, "Username Retrieved");
